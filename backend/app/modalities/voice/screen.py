"""End-to-end voice screening inference (reading-passage task).
...
Output is shaped to satisfy `app.modalities.base.ModalityResult` (the interface
every modality implements), on top of the original richer fields (narrative,
acoustic_report) kept for the clinician UI.
"""
...
from .model import load_model, MODEL_NAME, MODEL_VERSION
from .explain import explain_vector
...
def make_narrative(proba, band, families, confidence):
    ...
    if band == "low":
        lead = (f"This recording produced a screening indicator of {pct}%, which sits in the lower "
        ...
        lead = (f"This recording produced a screening indicator of {pct}%, which sits in the higher "
                f"range.")
        ...
        lead = (f"This recording produced a screening indicator of {pct}%, which sits in the middle "
                f"range and is not conclusive on its own.")
        mid = (f"A few patterns, such as {pd_str}, leaned toward the signals watched for, while "
               f"{hc_str} looked more typical.")
    conf = ("This estimate was steady across the recording." if confidence >= 0.66 else
            "The voice varied across the recording, so treat this estimate with extra caution.")
    tail = ("This is a quick screening from voice recordings and cannot diagnose any condition. "
            "Further clinical evaluation is recommended if this result is a concern.")
...
def _load_audio(wav, sr):
    if isinstance(wav, (str, Path)):
        ...
        return load_audio(wav, settings.sample_rate)
...
def _quality_gate(y):
    import librosa
    total = len(y) / settings.sample_rate
    ...
    if voiced < MIN_VOICED_SEC:
        return (False, f"Only {voiced:.0f}s of clear speech detected. Please read the full "
...
def _windows(y):
    win, hop = int(WIN_SEC * settings.sample_rate), int(HOP_SEC * settings.sample_rate)
    if len(y) <= win:
        return [y]
...
def _extract_windows(wav, sr=None):
    ...
    vecs = np.vstack([egemaps_signal(w, settings.sample_rate)[0] for w in _windows(y_trim)])
...
def _build_result(vecs, voiced_total, n_recordings) -> dict:
    ...
    shap_by_family = {f["family"]: f["shap"] for f in exp["all"]}
    acoustic_report = [{"label": lbl, "key": k, "value": round(float(name_val[k]), 4)}
                        for k, lbl in REPORT_KEYS if k in name_val]

    # Standardized biomarker list (base.Biomarker) for the generic Observation table.
    from .explain import family_of
    biomarkers = [
        {"code": k, "label": lbl, "value": round(float(name_val[k]), 4),
         "shap_contribution": round(float(shap_by_family.get(family_of(k), 0.0)), 4)}
        for k, lbl in REPORT_KEYS if k in name_val
    ]

    quality_score = round(min(1.0, voiced_total / RECOMMENDED_SEC), 2)
...
def screen(wav, sr: int | None = None) -> dict:
    ...
    if not ok:
        return {"ok": False, "modality": "voice", "task": "reading", "error": msg, "disclaimer": DISCLAIMER}
...
def screen_many(wavs, srs=None) -> dict:
    ...
    for i, wav in enumerate(wavs):
        ...
        if ok:
            ...
            errs.append(msg)
    if used == 0:
        return {"ok": False, "modality": "voice", "task": "reading",
                "error": errs[0] if errs else "No usable audio in the recordings.",
                "disclaimer": DISCLAIMER}
